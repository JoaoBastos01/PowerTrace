...
class BSPNode:
    ...
    def __init__(self, x: float, y: float, w: float, l: float, exterior_walls: Set[str] = None):
        ...
        # Fundamental para as Janelas! Rastreia as extremidades ao ar livre
    ...
    def split(self, split_ratio: float, horizontal: bool, rng: random.Random) -> bool:
        ...
        Atualiza magicamente as exterior_walls sem as paredes compartilhadas internas.
        """
        # Limiar mínimo tolerado por cômodo/corredor no Brasil (e.g. 1.0m)
        MIN_DIM = 1.0 
        
        if horizontal:
            # Corte Eixo Y (Fatia superior e inferior)
            split_length = self.length * split_ratio
            if split_length < MIN_DIM or (self.length - split_length) < MIN_DIM:
                return False  # Inválido, a fatia esticaria demais ou ficaria um filete cego
                
            left_w = self.width
            left_l = split_length
            
            right_w = self.width
            right_l = self.length - split_length
            right_y = self.y + split_length
            
            # O Nó "Left" (Sul do split horizontal) "perde" a visão do Norte se compartilhou parede
            ...
            self.left = BSPNode(self.x, self.y, left_w, left_l, left_ext)
            self.right = BSPNode(self.x, right_y, right_w, right_l, right_ext)
            
        else:
            # Corte Eixo X (Fatia Esquerda e Direita)
            ...
            right_w = self.width - split_width
            right_l = self.length
            right_x = self.x + split_width
            ...
            self.left = BSPNode(self.x, self.y, left_w, left_l, left_ext)
            self.right = BSPNode(right_x, self.y, right_w, right_l, right_ext)

        return True


class BSPTreeGenerator:
    """Orquestrador do Zoned BSP, que constrói a árvore de partições usando Target Areas e RNG determinístico."""
    ...
    def build_tree(self, rng: random.Random) -> Optional[List[BSPNode]]:
        """
        Executa o fatiamento guiado pelas áreas alvo e pelo rng determinístico.
        Retorna a lista de blocos arquiteturais brutos se gerou com sucesso, 
        ou None se as regras min_dimension travaram as linhas de corte.
        """
        root = BSPNode(0, 0, self.total_w, self.total_l)
        
        # MACRO ZONING - Fatiamento Primário (Garante Zonas separadas organicamente)
        ...
        if intimate_area > 0 and social_area > 0:
            # Decide o eixo de corte baseando-se em qual é mais vantajoso (ou via rng)
            horizontal_cut = rng.choice([True, False])
            split_ratio = social_area / total_area
            
            # Se falhar nesse eixo por restrição física, tenta no outro
            if not root.split(split_ratio, horizontal_cut, rng):
                horizontal_cut = not horizontal_cut
                if not root.split(split_ratio, horizontal_cut, rng):
                    return None 
            
            # Recurse recursivamente nas sub-listas de cada zona
            ...
            # Casa minúscula ou estúdio open concept (sem quartos)
    ...
    def _partition_node(self, node: BSPNode, rooms: Dict[str, float], rng: random.Random) -> bool:
        """Divide o espaço de forma recursiva pegando listas de cômodos até sobrar 1 p/ folha."""
        room_keys = list(rooms.keys())
        
        if len(room_keys) == 1:
            # Atingiu o caso base puro (1 bloco = 1 cômodo alvo)
            node.room_type = room_keys[0]
            return True
            
        # Randomiza (via RNG determinístico do seed de tentativa) qual bloco corta com qual
        rng.shuffle(room_keys)
        
        # Estratégia de divisão de arvore 50/50 em index
        mid = len(room_keys) // 2
        ...
        # Para evitar 'linguiças' extensas, forçamos o corte sempre cruzando o lado maior
